...
class Code(object):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                # Get joystick data
                joy_axes_raw = self.sub.get_data()['joy']['axes']
                joy_buttons = self.sub.get_data()['joy']['buttons']
                
                # print("joy axes array is", joy_axes_raw)
                # print("joy buttons array is", joy_buttons)
                
                #Pretvaramo vrijednosti od -1 do 1 do [1100,1900]
                joy_axes = [int(val*400 + 1500) for val in joy_axes_raw]
                print("joy axes array is : " ,joy_axes)
                
                pitch = 1500 #nije bitno, pitch se ne koristi
                roll = joy_axes[4]
                throttle = joy_axes[5]
                yaw = joy_axes[1]
                forward = joy_axes[3]
                lateral = joy_axes[2]
                reserved = 0 #ne koristi se 
                
                camera_tilt = 1500
                trokut = joy_buttons[0]
                krug = joy_buttons[1]
                iks = joy_buttons[2]
                kocka = joy_buttons[3]
                if(trokut == 1):
                ...
                override = [pitch, roll, throttle, yaw, forward, lateral, reserved, camera_tilt, lights, lights , 0, 0, 0, 0, 0, 0, 0, 0]
                print("Override array is: ", override)
                ...
                # for _ in range(len(override), 18):
                #     override.append(0)
